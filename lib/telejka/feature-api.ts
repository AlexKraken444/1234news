...
export async function featureApi(
  ...
  if (route === "uploads" && req.method === "POST") {
    ...
    if(!data.chatId&&data.mime.startsWith('audio/')){const [me]=await sql`SELECT plus_until>now() active FROM users WHERE id=${userId}`;if(!me?.active)throw new FeatureError(403,'Музыка доступна с TELEJKA+.');}
