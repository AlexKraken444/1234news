...
export async function economyApi(req:NextRequest,path:string[],input:Record<string,unknown>,userId:string){
 ...
 if(route==='market'&&req.method==='GET'){
  ...
  return reply(await sql`SELECT l.id,l.price::text price,l.seller_id,i.kind,i.seconds,telejka_user(l.seller_id,${userId}::uuid) seller FROM market_listings l JOIN reward_items i ON i.id=l.item_id WHERE l.status='open' ORDER BY l.created_at DESC,l.id LIMIT 40 OFFSET ${offset}`);
 }
 if(route==='rewards/spin'&&req.method==='POST'){
  ...
  const result=await sql.begin(async tx=>{
   ...
   if(!won){const [item]=await tx`INSERT INTO reward_items(owner_id,kind) VALUES(${userId},${prize}) RETURNING id`;itemId=item.id;}
