import { communityApi, canContact, canReadPost } from "@/lib/telejka/community-api";
...
import { db, databaseUrl, ensureDatabase } from "@/lib/telejka/db";
import { canManageVerification } from "@/lib/telejka/verification";
import { COOKIE, createSession, currentUser, hashToken } from "@/lib/telejka/auth";
import {
  registerSchema,
  profileSchema,
  bodySchema,
  ...
  nameSchema,
  passwordSchema,
} from "@/lib/telejka/validation";

import {
  featureApi,
  FeatureError,
  attachmentMetadata,
} from "@/lib/telejka/feature-api";
...
async function handle(
  ...
  try {
    ...
    if (!databaseUrl()) throw new ApiError(503,"Хранилище временно недоступно.");
    ...
    return json(
      { error: "Сервер временно недоступен. Попробуйте ещё раз.", reason: /compute.*(quota|limit)|(quota|limit).*compute/i.test(String((error as Error).message))?'COMPUTE_QUOTA':/storage|disk.*(full|quota)|size.*limit/i.test(String((error as Error).message))?'STORAGE_LIMIT':/suspend|disabled/i.test(String((error as Error).message))?'DATABASE_SUSPENDED':/too many|connection.*limit/i.test(String((error as Error).message))?'CONNECTION_LIMIT':'UNKNOWN', diagnostic: /^[A-Z0-9_]{2,32}$/.test(String((error as {code?:string}).code||"")) ? (error as {code?:string}).code : "UNAVAILABLE" },
      503,
    );
  }
}
